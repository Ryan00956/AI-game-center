...
export class WerewolfGame {
  constructor(container, app) {
    ...
    this.dayCount = 0;
    this.phase = 'night';
    this.messages = [];        // Public chat log (visible to all)
  ...
  renderSetup() {
    ...
    for (let i = 0; i < aiCount; i++) {
      if (!this.playerProfiles[i]) this.playerProfiles[i] = defaultProfile?.id || '';
    }
    ...
    document.getElementById('select-player-count')?.addEventListener('change', (e) => {
      ...
      for (let i = 0; i < n; i++) if (!this.playerProfiles[i]) this.playerProfiles[i] = def?.id || '';
      document.getElementById('ai-player-list').innerHTML = this.renderAISlots(n, aiService.profiles);
      this.bindSlots(n);
    });
    this.bindSlots(aiCount);
  ...
  renderAISlots(n, profiles) {
    return AI_NAMES.slice(0, n).map((name, i) => {
      const sid = this.playerProfiles[i] || '';
      const pi = profiles.findIndex(p => p.id === sid);
      const c = pi >= 0 ? PROFILE_COLORS[pi % PROFILE_COLORS.length] : '#666';
      return `<div class="ai-player-row">
        <div class="player-avatar">${AVATARS[i % AVATARS.length]}</div>
        <span class="player-name">${name}</span>
        <div class="model-color-indicator" style="background:${c}" id="dot-${i}"></div>
        <select class="form-input" id="profile-select-${i}">
          ${!profiles.length ? '<option value="">无可用模型</option>' : ''}
          ${profiles.map(p => `<option value="${p.id}" ${p.id === sid ? 'selected' : ''}>${p.name} (${p.model})</option>`).join('')}
        </select>
      </div>`;
    }).join('');
  }
  ...
  bindSlots(n) {
    for (let i = 0; i < n; i++) {
      ...
      if (sel) sel.addEventListener('change', (e) => {
        ...
        if (dot) dot.style.background = idx >= 0 ? PROFILE_COLORS[idx % PROFILE_COLORS.length] : '#666';
      });
    }
  }
  ...
  startGame() {
    ...
    for (let i = 0; i < this.playerCount; i++) {
      if (i === userPos) {
        this.players.push({
          ...
          privateMemory: [],   // role-specific private knowledge
        });
      } else {
    ...
    this.renderGameUI();
    this.sysMsg('游戏开始！所有玩家已就位。');
    this.privateMsg(`你的身份是：${ROLE_INFO[this.user.role].emoji} ${ROLE_INFO[this.user.role].name}`, 'important');
    if (this.user.role === 'werewolf') {
      const mates = this.players.filter(p => p.role === 'werewolf' && !p.isUser);
      if (mates.length) this.privateMsg(`你的狼人同伴是: ${mates.map(p => p.name).join(', ')}`, 'important');
    }
  ...
  renderGameUI() {
    this.container.innerHTML = `
      <div class="game-header">
        <div class="game-header-left">
          <button class="btn-back" id="btn-back">←</button>
          <div class="game-title-area"><h2>🐺 狼人杀</h2><span id="game-status-text">游戏进行中</span></div>
        </div>
        <span class="phase-indicator ${this.phase === 'night' ? 'night' : 'day'}" id="phase-badge">
          ${this.phase === 'night' ? '🌙 夜晚' : '☀️ 白天'}
        </span>
      </div>
      <div class="game-area">
        <div class="players-panel" id="players-panel"><h4>玩家列表</h4>${this.renderPlayerList()}</div>
        <div class="game-log" id="game-log">
          <div class="game-log-header"><span>游戏记录</span><span id="round-info">准备中</span></div>
    ...
    document.getElementById('btn-back')?.addEventListener('click', () => { if (confirm('退出游戏？')) this.app.navigate('home'); });
    const mc = document.getElementById('game-messages');
    this.messages.forEach(m => mc.appendChild(this.msgEl(m)));
  }
  ...
  refreshPlayers() {
    ...
    if (el) el.innerHTML = `<h4>玩家列表</h4>${this.renderPlayerList()}`;
  }

  setPhase(phase, label) {
    this.phase = phase;
    const b = document.getElementById('phase-badge');
    if (b) { b.className = `phase-indicator ${phase.includes('night') ? 'night' : 'day'}`; b.textContent = label; }
    const r = document.getElementById('round-info');
    if (r) r.textContent = `第 ${this.dayCount} 轮`;
  }
  ...
  sysMsg(text, type = '') {
    const m = { type: 'system', text, subtype: type };
    this.messages.push(m);
    const c = document.getElementById('game-messages');
    if (c) { c.appendChild(this.msgEl(m)); c.scrollTop = c.scrollHeight; }
  }

  /** Private message: shown in UI but NOT recorded in public log (AI won't see it) */
  privateMsg(text, type = '') {
    const m = { type: 'system', text, subtype: type };
    const c = document.getElementById('game-messages');
    if (c) { c.appendChild(this.msgEl(m)); c.scrollTop = c.scrollHeight; }
  }

  playerMsg(player, text) {
    const m = { type: 'player', player, text };
    this.messages.push(m);
    const c = document.getElementById('game-messages');
    if (c) { c.appendChild(this.msgEl(m)); c.scrollTop = c.scrollHeight; }
  }
  ...
  msgEl(m) {
    ...
    el.className = 'msg';
    if (m.type === 'system') {
      el.innerHTML = `<div class="msg-system ${m.subtype || ''}">${m.text}</div>`;
    } else {
      const u = m.player.isUser;
      const badge = !u && m.player.modelName ? `<span class="msg-model-badge">${m.player.modelName}</span>` : '';
      el.innerHTML = `<div class="msg-player ${u ? 'is-user' : ''}">
        <div class="msg-avatar">${m.player.avatar}</div>
        <div class="msg-body">
          <div class="msg-name">${m.player.name}${u ? ' (你)' : ''}${badge}</div>
  ...
  showThinking(p) {
    const c = document.getElementById('game-messages');
    if (!c) return;
    const mi = p.modelName ? ` (${p.modelName})` : '';
    const el = document.createElement('div');
    el.className = 'msg'; el.id = 'thinking';
    el.innerHTML = `<div class="thinking-indicator"><div class="thinking-dots"><span></span><span></span><span></span></div><span>${p.name}${mi} 正在思考...</span></div>`;
    c.appendChild(el); c.scrollTop = c.scrollHeight;
  }
  hideThinking() { document.getElementById('thinking')?.remove(); }
...
${player.role === 'seer' ? '- 你是预言家，你可以选择时机公开查验结果，但要注意暴露后可能被狼人针对。' : ''}`;
  ...
  /** Call AI with error handling. silent=true hides thinking indicator (for night actions) */
  async aiCall(player, situationPrompt, opts = {}) {
    ...
    try {
      const prompt = this.buildContext(player, situationPrompt);
      const resp = await aiService.chat(prompt, { temperature, maxTokens }, player.profileId);
      if (!silent) this.hideThinking();
      return resp;
    } catch (err) {
      if (!silent) this.hideThinking();
      if (!silent) this.sysMsg(`⚠️ ${player.name} (${player.modelName}) API失败: ${err.message}`, 'danger');
  ...
  async startNight() {
    ...
    this.sysMsg(`🌙 第 ${this.dayCount} 个夜晚降临了...`, 'important');
  ...
  async wolfNight() {
    ...
    while (!resolved && attempt < 3) {
      ...
      if (userIsWolf) {
        // ── User is wolf: discuss + vote (consistent with AI-only branch) ──
        const wolfChat = [];
        for (const wolf of wolves.filter(p => !p.isUser)) {
          ...
          if (resp) {
            this.privateMsg(`🐺 ${wolf.name}（队友）：${resp}`);
            wolfChat.push(`${wolf.name}：${resp}`);
            wolf.privateMemory.push(`第${this.dayCount}夜商议 - 你说：${resp}`);
          }
        }
        // All wolves vote (including user)
        this.privateMsg('🐺 请投票选择今晚要猎杀的目标');
        const userPick = await this.userChoice('投票猎杀目标', targets.filter(t => t.alive).map(t => ({ id: t.id, label: `${t.avatar} ${t.name}` })));
        ...
        if (tied.length === 1) {
          ...
          this.privateMsg(`🐺 投票一致，决定猎杀 ${this.nightKillTarget.name}`);
          for (const w of wolves) w.privateMemory.push(`第${this.dayCount}夜 - 狼人投票决定猎杀${this.nightKillTarget.name}`);
          resolved = true;
        } else {
          // Tie: re-discuss and re-vote (same as AI-only branch)
          this.privateMsg(`🐺 投票平票！${attempt < 3 ? '需要重新商议' : ''}`, 'important');
        ...
        for (const wolf of wolves) {
          ...
          // silent=true: no thinking indicator, so user can't see which AI is a wolf
  ...
  // ── Seer Night (hidden from non-seer users) ──
  ...
  async seerNight() {
    ...
    if (seer.isUser) {
      // User is seer: show the full experience
      this.privateMsg('🔮 预言家请睁眼，选择要查验的玩家');
      const target = await this.userChoice('选择查验目标', others.map(p => ({ id: p.id, label: `${p.avatar} ${p.name}` })));
      const checked = this.players[target];
      const isWolf = checked.role === 'werewolf';
      this.privateMsg(`查验结果：${checked.name} 是 ${isWolf ? '🐺 狼人！' : '✅ 好人'}`, isWolf ? 'danger' : 'success');
      seer.privateMemory.push(`第${this.dayCount}夜查验 - ${checked.name}是${isWolf ? '狼人' : '好人'}`);
    } else {
      // AI seer: decide silently (user sees nothing)
  ...
  async witchNight() {
    ...
    if (witch.isUser) {
      ...
      if (!this.witchHealUsed) {
        this.privateMsg(`🧙 女巫请睁眼。今晚 ${this.nightKillTarget.name} 被狼人猎杀了`);
        const heal = await this.userChoice('是否使用解药？', [
        ...
        if (heal === 'yes') {
          ...
          this.nightKillTarget = null;
          this.privateMsg('你使用了解药，被猎杀的玩家获救了');
        }
      }
      if (!this.witchPoisonUsed) {
        ...
        this.privateMsg('🧙 是否使用毒药？');
        const pt = await this.userChoice('选择毒药目标', poisonChoices);
        if (pt !== 'skip') {
          ...
          this.privateMsg(`你毒杀了 ${this.players[pt].name}`);
        }
      }
    } else {
      // --- AI Witch: all silent, user sees nothing ---
  ...
  async resolveNight() {
    ...
    // Collect all night deaths (wolf kill + witch poison) for unified resolution
    const nightDeaths = [];

    // Dedup: if poison target is the same as wolf kill target, poison is wasted
    ...
    // Announce all deaths (without revealing cause — standard werewolf rules)
    if (nightDeaths.length === 0) {
      this.sysMsg('昨晚是个平安夜，没有人死亡 🎉', 'success');
    } else {
      const names = nightDeaths.map(p => p.name).join('、');
      this.sysMsg(`昨晚 ${names} 死亡了！💀`, 'danger');
    }

    // Trigger hunter shot for any dead hunter (covers wolf kill AND poison death)
  ...
  // ════════════════════════════════════════════
  //  DAY PHASE (2 rounds of discussion + vote)
  // ════════════════════════════════════════════
  ...
  async startDay() {
    ...
    this.sysMsg(`☀️ 第 ${this.dayCount} 天 - 讨论第1轮：请每位玩家发表看法`, 'important');
    await this.discussionRound(1);
    if (this.state !== 'playing') return;

    // Round 2: Responses and challenges
    this.sysMsg(`💬 讨论第2轮：回应与质疑`, 'important');
    await this.discussionRound(2);
    if (this.state !== 'playing') return;

    // Vote (with re-vote on tie)
    await this.dayVote();
  }
  ...
  async userSpeak(roundNum) {
    this.sysMsg(`轮到你发言了${roundNum === 2 ? '（你可以回应其他人的发言）' : ''}`);
  ...
  async aiSpeak(player, roundNum) {
    ...
    if (resp) {
      this.playerMsg(player, resp);
    } else {
      this.playerMsg(player, this.fallbackSpeech(player));
    }
  ...
  async dayVote() {
    ...
    while (!resolved && attempt < 3) {
      ...
      if (attempt > 1) {
        this.sysMsg(`🗳 上轮投票平票！追加一轮讨论后重新投票`, 'important');
        await this.discussionRound(3); // Extra round
      } else {
        this.sysMsg('🗳 投票阶段！请选择要淘汰的玩家', 'important');
      }
      ...
      if (!Object.keys(tally).length) {
        ...
        this.sysMsg(`投票结果：${voteDetails}`);

        if (tied.length === 1) {
          const eliminated = this.players[parseInt(tied[0][0])];
          eliminated.alive = false;
          const ri = ROLE_INFO[eliminated.role];
          this.sysMsg(`${eliminated.name} 被淘汰了！身份是：${ri.emoji} ${ri.name}`, 'danger');
          if (eliminated.role === 'hunter') await this.hunterShot(eliminated);
          resolved = true;
        } else {
          // Tie
          const tiedNames = tied.map(([id]) => this.players[parseInt(id)].name).join('、');
          this.sysMsg(`${tiedNames} 之间平票！${attempt < 3 ? '将进行额外讨论后重新投票' : '本轮无人淘汰'}`, 'important');
          if (attempt >= 3) resolved = true; // Give up after 3 attempts
        }
      }
    }
  ...
  async hunterShot(hunter) {
    this.sysMsg(`🏹 ${hunter.name} 是猎人！可以带走一个人！`, 'important');
    const targets = this.alive.filter(p => p.id !== hunter.id);
    if (!targets.length) return;

    if (hunter.isUser) {
      const pick = await this.userChoice('猎人开枪', targets.map(p => ({ id: p.id, label: `🎯 ${p.avatar} ${p.name}` })));
      this.players[pick].alive = false;
      this.sysMsg(`猎人带走了 ${this.players[pick].name}！`, 'danger');
    } else {
      ...
      target.alive = false;
      this.sysMsg(`猎人 ${hunter.name} 开枪带走了 ${target.name}！`, 'danger');
    }
    this.refreshPlayers();
  }
  ...
  endGame(winner, message) {
    ...
    this.sysMsg(`🎉 游戏结束！${message}`, winner === 'village' ? 'success' : 'danger');
    this.refreshPlayers();
    ...
    overlay.innerHTML = `<div class="game-over-card">
      <div class="game-over-icon">${userWon ? '🎉' : '😢'}</div>
      <h2>${userWon ? '恭喜你赢了！' : '游戏失败'}</h2>
      <p>${message}</p>
      <p style="color:var(--text-muted);font-size:13px;">你的身份：${ri.emoji} ${ri.name}</p>
    ...
    document.getElementById('btn-play-again')?.addEventListener('click', () => { overlay.remove(); this.renderSetup(); });
    document.getElementById('btn-go-home')?.addEventListener('click', () => { overlay.remove(); this.app.navigate('home'); });
  }
  ...
  userChoice(title, options) {
    return new Promise(resolve => {
      const area = document.getElementById('action-area');
      if (!area) return;
      area.innerHTML = `<div class="action-panel"><h4>${title}</h4>
        <div class="action-buttons">${options.map(o =>
      ...
      area.querySelectorAll('.btn-vote').forEach(btn => {
        btn.addEventListener('click', () => {
          ...
          setTimeout(() => { area.innerHTML = ''; resolve(isNaN(c) ? c : parseInt(c)); }, 300);
        });
      });
      const mc = document.getElementById('game-messages');
